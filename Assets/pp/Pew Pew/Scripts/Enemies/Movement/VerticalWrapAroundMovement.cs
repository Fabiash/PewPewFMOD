...
namespace GameCore
{
    ...
    public class VerticalWrapAroundMovement : MonoBehaviour, AdjustableMoveSpeed, EnemyMove
    {
        ...
        void Start()
        {
            ...
            horizontalBounds = bounds.GetHorizontalBounds();
        ...
        void Update()
        {
            if (m_ShouldUpdate)
            {
                ...
                {
                    if (transform.position.x < bounds.GetHorizontalBounds().x)
                    {
                        if (GAME_MANAGER == null)
                        {
                            Destroy(gameObject);
                        ...
                        {
                            transform.position = new Vector2(bounds.GetHorizontalBounds().y, transform.position.y);
                        }
                    }
                }
                else
                {
                    if (transform.position.x > bounds.GetHorizontalBounds().y)
                    {
                        if (GAME_MANAGER == null)
                        {
                            Destroy(gameObject);
                        ...
                        {
                            transform.position = new Vector2(bounds.GetHorizontalBounds().x, transform.position.y);
                        }
                    }
                }
